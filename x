...
local function getDynamicFov(targetPosition)
    ...
    -- Check distance and return corresponding FOV radius based on the new naming convention
    if distance <= getgenv().Ranges.CloseDistance then
        return getgenv().BoundingSizes.Close
    elseif distance <= getgenv().Ranges.MidDistance then
        return getgenv().BoundingSizes.Mid
    elseif distance <= getgenv().Ranges.FarDistance then
        return getgenv().BoundingSizes.Far
    else
        return getgenv().BoundingSizes.Far  -- Default for targets beyond FarDistance
    end
end
...
local function updateFov()
    ...
    if IsTargeting and TargetPlayer then  
        ...
        if getgenv().Silent.FovShape == "Square" then
            ...
            for i = 1, 4 do
                ...
                line.Visible = getgenv().Silent.FovVisible
                line.From = corners[i]
                line.To = corners[i % 4 + 1]
                line.Color = getgenv().Silent.FovColor
                line.Thickness = getgenv().Silent.FovThickness
                line.Transparency = getgenv().Silent.FovTransparency
                table.insert(FovParts, line)
            end
        elseif getgenv().Silent.FovShape == "Triangle" then
            local points = {
            ...
            Fov.Visible = getgenv().Silent.FovVisible
            Fov.Radius = targetDistance  -- Set dynamic radius here
            Fov.Position = V2(Mouse.X, Mouse.Y + (G:GetService("GuiService"):GetGuiInset().Y))
            Fov.Color = getgenv().Silent.FovColor
            Fov.Thickness = getgenv().Silent.FovThickness
            Fov.Transparency = getgenv().Silent.FovTransparency
            Fov.Filled = getgenv().Silent.Filled
            if getgenv().Silent.Filled then
                Fov.Transparency = getgenv().Silent.FillTransparency
            end
        end
    else
